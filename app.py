...
# Flexible imports to prevent ModuleNotFoundError
try:
...
# 7. MAIN DASHBOARD
# ==========================================
st.markdown('<div class="badge-pill"><svg width="12" height="12" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" fill="#8B5CF6"/></svg> Gemini Vision Active</div>', unsafe_allow_html=True)
    ...
    if uploaded_video:
        if uploaded_video.name != st.session_state.get("last_uploaded_name"):
            if not client:
                ...
                with st.spinner("⚡ Uploading & indexing video on Gemini Cloud..."):
                    try:
                        ...
                        if USE_NEW_SDK:
                            ...
                            # CRITICAL: Wait until video processing is completed on Google Cloud
...
if st.button("🚀 Run AI Motion Breakdown", type="primary", use_container_width=True):
    if not client:
        ...
        st.error("❌ Please upload session footage first.")
    else:
        with st.spinner("Analyzing kinetic chain, weight transfer, and paddle path..."):
            try:
                target_clause = f"Focus on athlete: '{player_target}'." if (analysis_scope == "Target Specific Athlete" and player_target.strip()) else "Analyze all subjects."
                
                prompt = f"""Perform a professional pickleball biomechanics analysis:
                Athlete Experience Tier: {skill_level}/6.0
                Scope: {target_clause}
                
                Provide structured feedback on:
                1. Ready stance, knee flex & center of gravity
                2. Weight transfer & kinetic chain integration
                3. Paddle path, contact point & backswing length
                4. Actionable technical drills for improvement
                """
                ...
                if USE_NEW_SDK:
                    ...
                    response = model.generate_content([st.session_state.video_ref, prompt])

                st.session_state.analysis_text = response.text
            except Exception as e:
                st.session_state.analysis_text = f"⚠️ Analysis Failed: {str(e)}"
